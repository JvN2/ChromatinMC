    ...
    for i in range(n_steps):
        for bp in basepairs:
            rMC.MC_move(dna, bp, previous_bp, force, fixed_wrap_params, fixed_stack_params,
                        dyads, nucl, random_step, pars['e_wrap_kT'].value, pars['e_stack_kT'].value,
                        pars['fiber_start'].value)
            previous_bp = bp
        basepairs = basepairs[::-1]

        if i % 10 == 0:
            # g_stack_kt = rMC.score_stacking(dyads[0] + 1, dna.coords, dna.frames, dyads, fixed_stack_params, pars['e_stack_kT'].value,
            #                                 pars['fiber_start'].value)/kT
            g_stack_kt = 0
